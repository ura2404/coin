...
function def(){
    ...
    [ $PL -ne $PDEF ] && sudo $SMI -i $N -pl $PDEF &&  echo -e `date +%Y-%m-%d_%H:%M:%S`'\tNo.'$N'\t(*)def\t\t' $NP 'W' >> $LOG
}
...
function up(){
    ...
    [ $NP -lt $(( PMAX - DMAX )) ] && sudo $SMI -i $N -pl $NP && echo -e `date +%Y-%m-%d_%H:%M:%S`'\tNo.'$N'\t(^)up\t\t'$PL 'W -> '$NP 'W' >> $LOG
}
...
function down(){
    ...
    [ $PL -gt $(( PMIN +0 )) ] && sudo $SMI -i $N -pl $NP &&  echo -e `date +%Y-%m-%d_%H:%M:%S`'\tNo.'$N'\t(v)down\t\t'$PL 'W -> '$NP 'W' >> $LOG
}
